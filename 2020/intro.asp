...
<link href="../../defaultpage2019/css/main.css?<%=now()%>" rel="stylesheet">
<link href="../../defaultpage2019/css/main2.css?<%=now()%>" rel="stylesheet">
...
<script>
  ...
  document.documentElement.style.setProperty('--b-imageno', 'url(https://funday.asia/defaultpage2019/images/'+BgTime+')');
  BgTime=BgTime.replace('png','webp');
  document.documentElement.style.setProperty('--b-image', 'url(https://funday.asia/defaultpage2019/images/'+BgTime+')');

  // --------------------------------
  $(()=>{
    // 亂數給 video src v
    // let videoNo = ( String(new Date().getTime()) ).substr(12) % 3;
    // const videoUrl = 'https://test.funday.asia/defaultpage2020/video_' + videoNo + '.mp4';
    // const videoHtml = "<source src='" + videoUrl +"'>"
...
<div class="all">
    <header>
		<a href="https://funday.asia"><div class="Logo link" ></div></a>
		 <nav>
        <ul>
		<!-- Modify by Tom at 2019/6/5 -->
            ...
            <li onclick="fun_channel_ci('join');gtag('event', 'conversion', {'send_to': 'AW-748725678/WtDhCO_M2poBEK7LguUC'});"><span class="joinDiv Joinhide"> 免費註冊體驗</span></li>
		        <li id="llogin" data-toggle="popover"  data-placement="bottom" ><span class="LoginDiv">登入</span></li>
        </ul>
     </nav>     
		</header>
    <div id="videoBox">
      <video class='video-body' autoplay loop muted>
        <source src='https://test.funday.asia/defaultpage2019/video_0.mp4'>
      </video>
      ...
      <div id='videoText'>
        ...
        <div class="SButton <%=SRight%>" id='videoCta' onclick="fun_channel_ci('join');">免費加入</div>
      </div>
    </div>
    <div class="area1">
     <div class="ItemList">
      <div class="Slogn <%=SRight%>" ><%=Slogn%></div>
      <div class="SButton <%=SRight%>" >免費加入</div>
     </div>
    </div>
    <a id="area2"></a>
...
<img src="../../defaultpage2019/images2/FUNDAY_landing_page_logo_npuosat.png" />     
    ...
    <footer>
      <div class="ItemList">
       ...
       <div class="crop"> © 2020 Brainstorm Digital Communications Corp. All rights reserved.Privacy Policy </div>
      </div>
    </footer>
</div>
<script src="../../defaultpage2019/js/main.js?a<%=now()%>" ></script>